...
다음 개발판 `1.9.0-beta.8`은 SafeDOM의 비동기 전역 클릭과 라디오 기본 동작 사이 경합을 제거하여 아이템 상세 카드 지연 로딩을 안정화합니다. `1.9.0-beta.7`에서 추가한 설정 선로딩, 전체 시각 이펙트 토글, 부드러운 조우 스캔, 스킬·조우 이모지 UI도 포함합니다. 이전 `1.9.0-beta.6`에서 도입한 닫힌 패널의 가벼운 골격도 그대로 유지하여 배지 클릭 즉시 화면을 열고,
준비된 채팅 스냅숏과 기존 SafeDOM 루트를 재사용한다.

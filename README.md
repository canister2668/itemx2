...
현재 공개 버전은 `2.0.18`입니다. 플러그인 목록에도 `ITEMX CODEX · v2.0.18`로 표시되므로 설치된 버전과 업데이트 여부를 바로 확인할 수 있습니다.
...
2. ITEMX 기록이 없는 새 채팅을 열고 같은 화면에서 파일을 선택하거나 텍스트를 붙여넣습니다.
3. **내용 확인**으로 원본 캐릭터와 항목 수를 확인한 뒤 **이 채팅에 불러오기**를 누릅니다.

아이템 수량·장착·속성, 스킬 숙련도, 조우 결과·초상 이름, 기록의 보존·보관 설정과 경과 입력 수를 가져옵니다. 이후 대화부터 기존 기록을 기준으로 추적을 이어갑니다. 기존 ITEMX 기록이 있으면 덮어쓰지 않고 새 채팅을 안내합니다.

...
현재 공개 베타는 `1.9.0-beta.20`이다. Risu 플러그인 목록에는 `ITEMX CODEX · v1.9.0-beta.20`으로 표시되어 설치·업데이트 버전을 바로 확인할 수 있다. 제품명은 `ITEMX CODEX`, 부제는 `World Inventory & Encounter Archive`다. ITEMX/CODEX 전송 마커는 process 단계와 실제 모델 요청 직전에 제거하고, 현재 상태는 별도 권위 스냅숏으로만 전달한다. 스킬 상세은 아이템·무기 카드와 같은 속성·파티클 FX 엔진을 사용하며, 스킬 등급 수집과 내부 시각 등급은 설정의 `세계관 우선 / ITEMX 강제` 기준에 함께 연동된다. 최초 등록된 스킬은 본문 맥락에서 경력과 숙련도를 보수적으로 추론하되, 모델이 명시한 수치를 우선하고 추론 출처를 내부에 구분한다. 라이트보드의 표준 LBDATA는 ITEMX 보조 복구 문맥과 재검사 해시에서 제외하여 데이터 갱신만으로 보조 호출이 반복되지 않는다. 조우는 붉은 아카이브 디자인을 유지하면서 마지막으로 확정된 전투의 결말과 결정적 해결 방식을 별도 결과 영역에 표시한다. 설정의 채팅별 UI 글자 크기는 전체 재렌더 없이 즉시 적용된다.

모델이 고른 정상 이모지는 그대로 보존하고, 누락·오류·`❔` 아이콘만 아이템·스킬·조우 유형에 맞는 결정론적 폴백으로 교체한다. 선택형 `모듈 에셋 초상화`는 허용된 DB 범위에서 현재 활성 모듈의 에셋만 탐색하며 AVIF를 지원한다. 모델이 초상화를 `NONE`으로 남겨도 인물 ID·이름·별칭에 맞는 `standing`, `default`, `neutral` 계열 대표 에셋을 우선 선택하고, 권한·API·조회·이미지 로딩 중 어느 단계가 실패해도 물음표 대신 조우 유형별 이모지로 폴백한다. 기존 SafeDOM 클릭 라우터, 설정 선로딩, 시각 이펙트 토글, 지연 상세 로딩과 스크롤 최적화도 그대로 포함한다.

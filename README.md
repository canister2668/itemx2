...
현재 공개 베타는 `1.9.0-beta.11`이다. 제품명은 `ITEMX CODEX`, 부제는 `World Inventory & Encounter Archive`다. 같은 보조 처리에서 한 아이템의 최초 감정과 상세 보완이 연속해도 사건 원장은 보존하면서 본문에는 최종 카드 한 장만 표시한다. 이미 배운 플레이어 스킬도 본문에서 소유·사용·숙련 사실이 처음 명확히 확정되면 발견 사건으로 수집하며, 적의 기술은 플레이어 스킬이 아닌 조우 도감 기술 목록에만 보존한다.

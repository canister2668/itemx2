...
현재 공개 베타는 `1.9.0-beta.15`이다. Risu 플러그인 목록에는 `ITEMX CODEX · v1.9.0-beta.15`로 표시되어 설치·업데이트 버전을 바로 확인할 수 있다. 제품명은 `ITEMX CODEX`, 부제는 `World Inventory & Encounter Archive`다. ITEMX/CODEX 전송 마커는 process 단계와 실제 모델 요청 직전에 제거하고, 현재 상태는 별도 권위 스냅숏으로만 전달한다. 스킬 효과는 공통 원형 궤도를 사용하지 않고 화염·냉기·번개·암흑·빛·비속성마다 서로 다른 형태 언어를 사용하며, 조우는 종족·위협·관계별 저비용 목록 효과와 상세 효과를 사용한다. 설정에는 채팅별 UI 글자 크기 소·중·대가 추가되며 메인 패널에서는 전체 재렌더 없이 즉시 적용된다. 가장 최근 사건 출력만 첫 화면 복원용 표시 정보를 유지하고 이전 출력은 짧은 ITEMX/CODEX 참조로 축약한다. 설정의 현재 채팅 정리 기능은 현재 봇을 OFF로 전환한 뒤 해당 채팅의 본문 마커와 플러그인 원장을 제거한다.

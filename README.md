...
현재 공개 베타는 `1.9.0-beta.18`이다. Risu 플러그인 목록에는 `ITEMX CODEX · v1.9.0-beta.18`로 표시되어 설치·업데이트 버전을 바로 확인할 수 있다. 제품명은 `ITEMX CODEX`, 부제는 `World Inventory & Encounter Archive`다. ITEMX/CODEX 전송 마커는 process 단계와 실제 모델 요청 직전에 제거하고, 현재 상태는 별도 권위 스냅숏으로만 전달한다. 스킬 상세은 아이템·무기 카드와 같은 속성·파티클 FX 엔진을 사용하며, 스킬 등급 수집과 내부 시각 등급은 설정의 `세계관 우선 / ITEMX 강제` 기준에 함께 연동된다. 최초 등록된 숙련 스킬은 본문의 이름·계통별 수치 근거로 레벨을 교정하며, 근거 없는 `Lv.1 / 숙련 0%`는 미상으로 보존한다. 라이트보드의 표준 LBDATA는 ITEMX 보조 복구 문맥과 재검사 해시에서 제외하여 데이터 갱신만으로 보조 호출이 반복되지 않는다. 조우는 붉은 아카이브 디자인을 유지하면서 마지막으로 확정된 전투의 결말과 결정적 해결 방식을 별도 결과 영역에 표시한다. 설정의 채팅별 UI 글자 크기는 전체 재렌더 없이 즉시 적용된다.

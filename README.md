...
ITEMX CODEX는 해적리스 채팅에서 얻은 아이템, 익힌 스킬, 실제로 맞선 상대를 자동으로 정리해 주는 RisuAI API v3 플러그인입니다. 대화 중 중요한 변화가 생겼을 때만 카드를 보여주고, 평소에는 화면 옆 `CODEX` 배지에서 인벤토리와 도감을 편하게 열어볼 수 있습니다.

현재 공개 버전은 `1.9.0-beta.30`입니다. 플러그인 목록에도 `ITEMX CODEX · v1.9.0-beta.30`으로 표시되므로 설치된 버전과 업데이트 여부를 바로 확인할 수 있습니다.

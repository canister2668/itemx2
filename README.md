...
현재 공개 베타는 `1.9.0-beta.16`이다. Risu 플러그인 목록에는 `ITEMX CODEX · v1.9.0-beta.16`로 표시되어 설치·업데이트 버전을 바로 확인할 수 있다. 제품명은 `ITEMX CODEX`, 부제는 `World Inventory & Encounter Archive`다. ITEMX/CODEX 전송 마커는 process 단계와 실제 모델 요청 직전에 제거하고, 현재 상태는 별도 권위 스냅숏으로만 전달한다. 스킬 효과는 화염·냉기·번개·암흑·빛·비속성별로 열기, 결정, 방전, 심연, 후광, 룬 파편의 서로 다른 재질과 실루엣을 사용한다. 조우는 야수·언데드·구조체·용·수생·곤충·인형/미상을 로컬로 분류하고, 종류 재질과 전투 경고를 독립 레이어로 표현한다. 목록은 항목당 장식 DOM 하나만 사용하고, 상세는 고정 레이어 네 개로 제한한다. 설정의 채팅별 UI 글자 크기는 전체 재렌더 없이 즉시 적용된다.

...
현재 공개 베타는 `1.9.0-beta.13`이다. Risu 플러그인 목록에는 `ITEMX CODEX · v1.9.0-beta.13`으로 표시되어 설치·업데이트 버전을 바로 확인할 수 있다. 제품명은 `ITEMX CODEX`, 부제는 `World Inventory & Encounter Archive`다. 가장 최근 사건 출력만 첫 화면 복원용 표시 정보를 유지하고 이전 출력은 짧은 ITEMX/CODEX 참조로 축약한다. 설정의 현재 채팅 정리 기능은 현재 봇을 OFF로 전환한 뒤 해당 채팅의 본문 마커와 플러그인 원장을 제거한다. 같은 보조 처리에서 한 아이템의 최초 감정과 상세 보완이 연속해도 사건 원장은 보존하면서 본문에는 최종 카드 한 장만 표시하며, 이미 배운 플레이어 스킬도 소유·사용·숙련 사실이 처음 명확히 확정되면 발견 사건으로 수집한다.

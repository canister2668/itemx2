...
현재 공개 베타는 `1.9.0-beta.5`다. 이번 판은 첫 진입 카드 복원과 채팅 스크롤 성능을 구조적으로 고쳤다.
새 출력의 짧은 참조는 표시용 정보를 자체 포함하며, 구형 대화의 참조도 저장된
사건 원장에서 한 번만 승격한다. 메인 DOM 동기화는 주기적 전체 탐색 대신 변경 관찰자로 처리하고, 스크롤 중
카드별 위치 계산과 연속 브리지 호출을 하지 않는다. 원격 헤더 2KB만 30분 간격으로 확인하는 업데이트 표식도 포함한다.
...
- 플러그인 로드 직후 권한 처리보다 먼저 화면 측면의 `💎 ITEMX` 배지를 등록한다.
...
이 디렉터리가 ITEMX 2 소스 저장소다. 다른 제품 저장소의 파일을 빌드 입력으로 사용하지 않는다.

...
현재 공개 버전은 `1.9.0-beta.26`입니다. 플러그인 목록에도 `ITEMX CODEX · v1.9.0-beta.26`으로 표시되므로 설치된 버전과 업데이트 여부를 바로 확인할 수 있습니다.
...
모델이 출력 형식을 놓친 경우에는 보조 복구가 필요한 부분만 다시 확인합니다. 스트리밍 중에는 실행하지 않고, 여러 기록이 한꺼번에 빠졌을 때도 한 번에 묶어서 처리합니다. 라이트보드의 표준 데이터 갱신은 복구 대상으로 오인하지 않습니다.

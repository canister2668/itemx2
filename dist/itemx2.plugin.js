...
`;
  ...
  function panelMenuHtml(native = true, enabled = true) {
    return `<div class="itemx2-panel-actions"><button class="itemx-ph-btn itemx2-sw-power itemx2-setting-toggle${enabled ? ' itemx2-power-on' : ''}" data-action="toggle" type="button" role="switch" aria-checked="${enabled ? 'true' : 'false'}" aria-label="이 봇에서 사용" title="이 봇에서 사용">사용</button><label class="itemx-ph-btn itemx2-search-open" for="itemx2-search-toggle" role="button" aria-label="검색" title="검색">🔍</label><button class="itemx-ph-btn itemx2-history-open" data-action="history-open" type="button" aria-label="기록 보기" title="기록 보기">기록</button><button class="itemx-ph-btn ${native ? 'itemx2-root-close' : ''}" data-action="close" type="button" aria-label="닫기" title="닫기">✕</button></div>`;
  }

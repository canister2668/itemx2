...
const ITEMXLorebook = (() => {
  ...
  function publicFields(entry) {
    ...
    for (const line of source.split(/\r?\n/).slice(0, 80)) {
      const match = line.match(/^\s*(?:[-*]\s*)?(aliases?|별칭|kind|type|종류|portrait|초상화|description|소개|설명)\s*[:：]\s*(.+?)\s*$/i);
      if (!match) continue;
      const key = match[1].toLowerCase(), value = text(match[2], key === 'description' || key === '소개' || key === '설명' ? 600 : 200);
      if (!value) continue;
  ...
  function candidate(entry, index) {
    ...
    const keys = entryKeys(entry), comment = text(entry.comment || entry.name, 160);
    const labels = [...new Set([...keys, comment].filter(Boolean))];
    return { id: entryId(entry, index), comment, labels, norms: [...new Set(labels.map(normalize).filter(Boolean))], fields: publicFields(entry) };
  }
  ...
  function index(entries) {
    ...
    (Array.isArray(entries) ? entries : []).forEach((entry, entryIndex) => {
      ...
      for (const name of row.norms) {
        ...
        rows.push(row); byName.set(name, rows);
      }
    });
    return byName;
  }
  ...
  function emptyLedger() { return { v: VERSION, rows: {}, updatedAt: 0 }; }
  ...
  function scan(snapshot, entries, previous, onlyId = '') {
    ...
    const lookup = index(entries), result = { matched: 0, enriched: 0, removed: 0, unchanged: 0, ambiguous: 0, notFound: 0 };
    ...
    for (const id of monsters?.order || []) {
      ...
      if (found.status === 'ambiguous') {
        result.ambiguous += 1;
        if (ledger.rows[id]) { delete ledger.rows[id]; result.removed += 1; }
        continue;
      }
      ...
      else { ledger.rows[id] = next; result.enriched += 1; }
    }
    const ids = Object.keys(ledger.rows);
    if (ids.length > MAX_ROWS) ids.sort((a, b) => (ledger.rows[b]?.at || 0) - (ledger.rows[a]?.at || 0)).slice(MAX_ROWS).forEach((id) => delete ledger.rows[id]);
    ledger.updatedAt = Date.now();
    return { ledger, result };
  }

  function apply(snapshot, ledger) {
    const next = clone(snapshot || {}), monsters = next?.monsters;
    if (!monsters?.entries) return next;
    for (const id of monsters.order || []) {
      const monster = monsters.entries[id], row = ledger?.rows?.[id];
      if (!monster || !row?.fields) continue;
      const fields = row.fields;
      if (Array.isArray(fields.aliases) && fields.aliases.length) monster.aliases = [...new Set([...(monster.aliases || []), ...fields.aliases])].slice(0, 12);
    ...
    const ledgerView = Object.entries(ledger?.rows || {}).map(([id, row]) => [id, row?.fingerprint || '', row?.source || '']);
  ...
  function hash(value) {
    let out = 0x811c9dc5;
    for (const char of String(value || '')) { out ^= char.charCodeAt(0); out = Math.imul(out, 0x01000193); }

...
/* Body layers sit under the footer text, not over it. */
.itemx2-inline-appraisal .itemx2-inline-foot{position:relative;z-index:1}
/* Readable kicker and state on phones. */
...
/* Must follow the v6 block above, which otherwise overrides the compact phone layout. */
...
`;
  ...
  // Only animation-play-state is safe to toggle while scrolling. visibility,
  // box-shadow, filter and mix-blend-mode each change what the compositor has to
  // build: switching filter or blend on or off creates and destroys a stacking
  // context, so the browser re-rasterises the whole .chattext column and every
  // other module's HTML inside it blinks along with our cards. Pausing an
  // animation changes no geometry, no paint and no layer, so motion just stops.
  ...
  function skinCss(name) {
    ...
    return [
      ...
      // Inline chat cards were tuned for dark only; on paper skins their cream text vanished.

...
const fixture = bundle.replace(anchor, `
  globalThis.itemxAudit = { async setup(tab, skin) {
    ...
    const items = names.map((name,i) => ITEMXCore.normalizeItem({
      ...
      affinity: globalThis.ITEMX_AFF || ['fire','ice','lightning','wind','earth','dark','light','poison'][i],
    ...
    root.innerHTML=prefixMarkup(items.slice(0,2).map(i=>ITEMXRenderer.renderCard(i,{motion: globalThis.ITEMX_MOTION || 'full'})).join(''));

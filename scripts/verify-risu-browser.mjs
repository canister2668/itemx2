...
const [core, renderer, css, purify] = await Promise.all([
  ...
  readFile(resolve(root, 'dist/itemx2-main-scoped.css'), 'utf8'),
  readFile(purifyPath, 'utf8')
]);
...
await page.screenshot({
  path: process.env.ITEMX_SCREENSHOT || resolve(root, 'dist/risu-pipeline-mobile.png'),
  fullPage: true
});

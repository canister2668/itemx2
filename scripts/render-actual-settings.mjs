...
const Risuai = {
  ...
  addRisuScriptHandler: idle,
  removeRisuScriptHandler: idle,
  unregisterUIPart: idle,
  onUnload: idle,
  hideContainer: idle
};
...
const loaded = {
  ...
  key: 'guide:settings',
  snapshot: {
    fingerprint: 'guide-items',
    registry: { order: items.map((one) => one.id), items: Object.fromEntries(items.map((one) => [one.id, one])) }
  },
  codexSnapshot: {
...
const prefixMarkupClasses = (html) =>
  html.replace(
    /class="([^"]*)"/g,
    (_raw, value) =>
      `class="${value
        .split(/\s+/)
        .filter(Boolean)
        .map((name) => (name.startsWith('x-risu-') ? name : `x-risu-${name}`))
        .join(' ')}"`
  );
...
const inlineCards = [
  guide.codexInlineEventHtml(
    {
      ...
      view: {
        ...
        mastery: 92,
        cost: '지속적인 기운 소모',
        cooldown: '형태 전환 시 찰나의 호흡 필요',
        target: '자신이 파지한 도검',
        affinity: 'lightning',
        effects: ['도검에 불안정한 전격을 부여', '연속 타격 시 방전 강화']
      }
    },
    'off'
  ),
  guide.codexInlineEventHtml(
    {
      event: { domain: 'monster', kind: 'exam' },
      view: {
        id: 'abyss_beast',
        name: '심연의 괴수',
        glyph: '👾',

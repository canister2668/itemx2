...
await page.evaluate(() => {
  ...
  const wrap = (node) =>
    node
      ? {
          ...
          setClassName: async (value) => {
            node.className = value;
          },
          setTextContent: async (value) => {
            node.textContent = value;
          },
          setInnerHTML: async (value) => {
  ...
  window.Risuai = {
    ...
    registerButton: async (arg, callback) => {
      const hostButton = document.createElement('button');
      hostButton.setAttribute('aria-label', arg.name);
      hostButton.innerHTML =
        arg.iconType === 'img' ? `<div><img src="${arg.icon}" alt=""></div>` : `<div>${arg.icon}</div>`;
      document.body.appendChild(hostButton);
      window.__itemxUiTest.registrations.push({ type: 'button', arg, callback });
      return { id: arg.id };
    },
    registerSetting: async (name, callback, icon, iconType, id) => {
      window.__itemxUiTest.registrations.push({ type: 'setting', name, callback });
      return { id };
    },
    requestPluginPermission: async () => true,
    addRisuScriptHandler: async (mode, callback) => {
      window.__itemxUiTest.handlers[mode] = callback;
    },
    removeRisuScriptHandler: async () => {},
    addRisuReplacer: async (mode, callback) => {
      window.__itemxUiTest.replacers[mode] = callback;
    },
    removeRisuReplacer: async () => {},
    addRisuChatListener: async (mode, callback) => {
      window.__itemxUiTest.listeners[mode] = callback;
    },
    unregisterUIPart: async () => {},
    getRootDocument: async () => safeDocument,
    onUnload: async () => {},
    alertError: async (message) => {
      window.__itemxUiTest.alert = message;
    },
    showContainer: async (type) => {
      window.__itemxUiTest.showCalls += 1;
      window.__itemxUiTest.requestedModes.push(type);
      window.__itemxUiTest.container = type;
    },
    resizeContainer: async (height, width) => {
      window.__itemxUiTest.size = { height, width };
      if (window.__itemxUiTest.resizeFails) throw new Error('API method resizeContainer not found');
    },
    hideContainer: async () => {
      window.__itemxUiTest.hideCalls += 1;
      window.__itemxUiTest.container = 'hidden';
    }
  };
});
...
const badge = await page.evaluate(() => {
  ...
  return {
    ...
    image: button.querySelector('img')?.src.startsWith('data:image/svg+xml')
  };
});
if (badge.width !== 48 || badge.height !== 176 || badge.text !== '' || !badge.image)
  throw new Error(`vertical badge failed: ${JSON.stringify(badge)}`);
...
const drawer = await page.evaluate(() => {
  ...
  return {
    ...
    filters: [...panel.querySelectorAll('.itemx2-root-filters label')].map(
...
if (
  drawer.rootPointer !== 'none' ||
  drawer.panelPointer !== 'auto' ||
  drawer.width >= 390 ||
  drawer.height >= 844 ||
  drawer.title !== 'UI 시험 봇' ||
  drawer.filters.join(',') !== '전체,보유,장착,관찰,소실' ||
  drawer.showCalls !== 0 ||
  drawer.container !== 'hidden'
)
  throw new Error(`nonblocking root drawer failed: ${JSON.stringify(drawer)}`);
...
const closed = await page.evaluate(() => ({
  checked: document.querySelector('#itemx2-root-open')?.checked,
  visibility: getComputedStyle(document.querySelector('.itemx2-root-layer')).visibility,
  showCalls: window.__itemxUiTest.showCalls
}));
if (closed.checked || closed.visibility !== 'hidden' || closed.showCalls !== 0)
  throw new Error(`native close failed: ${JSON.stringify(closed)}`);
...
if (streaming.checked || streaming.showCalls !== 0)
  throw new Error(`streaming reopened drawer: ${JSON.stringify(streaming)}`);

await page.evaluate(async () => window.__itemxUiTest.registrations.find((entry) => entry.type === 'button').callback());
await page.waitForFunction(() => document.querySelector('#itemx2-root-open')?.checked === true);
const reopened = await page.evaluate(() => ({
  roots: document.querySelectorAll('[x-itemx2-drawer="owner"]').length,
  checked: document.querySelector('#itemx2-root-open')?.checked,
  showCalls: window.__itemxUiTest.showCalls
}));
if (reopened.roots !== 1 || !reopened.checked || reopened.showCalls !== 0)
  throw new Error(`drawer recreate failed: ${JSON.stringify(reopened)}`);

await page.evaluate(async () =>
  window.__itemxUiTest.registrations.find((entry) => entry.type === 'setting').callback()
);
await page.waitForFunction(() => window.__itemxUiTest.showCalls === 1);
const settings = await page.evaluate(() => ({
  mode: window.__itemxUiTest.container,
  requested: window.__itemxUiTest.requestedModes.at(-1),
  active: document.querySelector('.itemx-main-tab-on')?.textContent.trim(),
  fallback: document.querySelector('.itemx-plugin-stage')?.classList.contains('itemx-plugin-stage-fallback')
}));
if (
  settings.mode !== 'fullscreen' ||
  settings.requested !== 'fullscreen' ||
  settings.active !== '설정' ||
  !settings.fallback
)
  throw new Error(`explicit settings fallback failed: ${JSON.stringify(settings)}`);

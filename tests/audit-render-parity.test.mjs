...
test('audit fixes preserve f289864 nonempty panel HTML and all affinity card markup',async()=>{
 ...
 const norm=h=>moved(h).replace(/--w:[\d.]+%/g,'RAYW').replace(/<style>[\s\S]*?<\/style>/g,'STYLE').replace(/\.itemx2-panel-actions\s*\{[^}]*\}\s*\.itemx2-panel-actions > button(,\s*\.itemx2-panel-actions > label)?\s*\{[^}]*\}/g,'ACTIONS_BOX');

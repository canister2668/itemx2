...
test('audit fixes preserve f289864 nonempty panel HTML and all affinity card markup',async()=>{
 ...
 const fxcard=h=>h.replace(/<section class="itemx2-root-setting-card"><span><strong>이펙트[\s\S]*?<\/section>/g,'');
 const motes=h=>h.replace(/<i class="craft-mote[^>]*><\/i>/g,'').replace(/<i style="--x:[^>]*><\/i>/g,'')
   .replace(/<b style="--x:[^>]*><\/b>/g,'');
 const norm=h=>fxcard(motes(moved(h))).replace(/<div class="itemx2-feedback" role="status" aria-live="polite"><\/div>/g,'').replace(/(ITEMX(?: CODEX)?(?: ·)? )\d+\.\d+\.\d+/g,'$1VERSION').replace(/ITEMX CODEX/g,'ITEMX').replace(/CODEX(?= 배지|에서 확인|에 기록)/g,'ITEMX').replace(/CODEX · APPRAISAL/g,'ITEMX · APPRAISAL').replace(/<div class="itemx2-native-badge" x-itemx2-badge="launcher" aria-label="ITEMX">(?:<img [^>]*>|<span class="itemx2-badge-seal">[\s\S]*?<span class="itemx2-badge-foot">[\s\S]*?<\/span>)/g,'BADGE').replace(/--w:[\d.]+%/g,'RAYW').replace(/<style>[\s\S]*?<\/style>/g,'STYLE').replace(/\.itemx2-panel-actions\s*\{[^}]*\}\s*\.itemx2-panel-actions > button(,\s*\.itemx2-panel-actions > label)?\s*\{[^}]*\}/g,'ACTIONS_BOX');
 ...
 const loaded={key:'c:chat',enabled:true,mainOutput:true,auxOutput:'off',rarityMode:'world',itemsEnabled:true,skillsEnabled:true,encountersEnabled:true,lorebookEncounterEnabled:false,moduleAssetsEnabled:false,effectsLevel:'full',effectsEnabled:true,debugEnabled:false,fontScale:'small',character:{name:'Audit'},chat:{message:[],scriptstate:{}},snapshot:{registry:{order:items.map(i=>i.id),items:Object.fromEntries(items.map(i=>[i.id,i])),diagnostics:[]},history:{},fingerprint:'stable'},codexSnapshot:codex};

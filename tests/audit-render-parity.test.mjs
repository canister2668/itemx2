...
test('audit fixes preserve f289864 nonempty panel HTML and all affinity card markup',async()=>{
 ...
 for(const skin of ['dark','light'])for(const tab of ['inventory','skills','bestiary','settings'])for(const open of [true,false])assert.equal(next.rootInventoryHtml({...loaded,skin},open,tab),old.rootInventoryHtml({...loaded,skin},open,tab));
 for(const item of items){const marker=next.core.marker({v:2,event:{kind:'exam',item},view:item});assert.equal(next.displayHandler(marker),old.displayHandler(marker));}
});

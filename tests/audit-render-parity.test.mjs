...
test('audit fixes preserve f289864 nonempty panel HTML and all affinity card markup',async()=>{
 ...
 const norm=h=>fxcard(motes(moved(h))).replace(/ITEMX CODEX/g,'ITEMX').replace(/CODEX(?= 배지|에서 확인|에 기록)/g,'ITEMX').replace(/CODEX · APPRAISAL/g,'ITEMX · APPRAISAL').replace(/<div class="itemx2-native-badge" x-itemx2-badge="launcher" aria-label="ITEMX">(?:<img [^>]*>|<span class="itemx2-badge-seal">[\s\S]*?<span class="itemx2-badge-foot">[\s\S]*?<\/span>)/g,'BADGE').replace(/--w:[\d.]+%/g,'RAYW').replace(/<style>[\s\S]*?<\/style>/g,'STYLE').replace(/\.itemx2-panel-actions\s*\{[^}]*\}\s*\.itemx2-panel-actions > button(,\s*\.itemx2-panel-actions > label)?\s*\{[^}]*\}/g,'ACTIONS_BOX');

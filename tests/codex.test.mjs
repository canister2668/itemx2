...
test('encounter endings and skill mastery are deterministic patches', () => {
  const first = codex.extractResponse(
    '<skillExam><id>moon_slash</id><name>월영참</name><type>active</type><status>learned</status></skillExam><monsterExam><id>wolf_king</id><name>흑랑왕</name><relation>hostile</relation><status>active</status></monsterExam>',
    codex.snapshot()
  );
  const next = codex.extractResponse(
    '<skillPatch><id>moon_slash</id><action>mastery</action><mastery>88</mastery></skillPatch><monsterPatch><id>wolf_king</id><action>defeat</action></monsterPatch>',
    first.snapshot
  );
  ...
  const payloads = [...next.content.matchAll(/<!--CODEX2:([A-Za-z0-9_-]+)-->/g)].map((match) =>
    codex.decodePayload(match[1])
  );
...
test('latest completed encounter outcome records the decisive resolution without accumulating history', () => {
  const first = codex.extractResponse(
    '<monsterExam><id>wolf_king</id><name>흑랑왕</name><relation>hostile</relation><status>active</status></monsterExam>',
    codex.snapshot()
  );
  const defeated = codex.extractResponse(
    '<monsterPatch><id>wolf_king</id><action>defeat</action><outcome>연화검의 결정타가 앞발을 부러뜨리며 흑랑왕을 무릎 꿇렸다.</outcome></monsterPatch>',
    first.snapshot
  );
  ...
  const again = codex.extractResponse(
    '<monsterPatch><id>wolf_king</id><action>encounter</action></monsterPatch>',
    defeated.snapshot
  );
  ...
  const killed = codex.extractResponse(
    '<monsterPatch><id>wolf_king</id><action>kill</action><outcome>두 번째 교전에서 월영참이 심장을 관통했다.</outcome></monsterPatch>',
    again.snapshot
  );
...
test('character catalogs keep internal additional assets, ccAssets and emotion images', () => {
  ...
  names.push(
    ['later_hero_default', 'asset-hero-default', 'webp'],
    ['later_hero_annoyed', 'asset-hero-annoyed', 'webp']
  );
  ...
  assert.ok(
    rows.some((row) => row.name === 'later_hero_default'),
    'bot additionalAssets after the old 400-name prefix stay in catalog'
  );
  assert.equal(codex.assetLookup(rows, 'cc_portrait_default')?.id, 'assets/cc-portrait.webp');
  assert.equal(
    codex.assetForEntity(rows, { id: 'later_hero', name: '후반 히어로', aliases: [], portrait: 'NONE' })?.id,
    'asset-hero-default'
  );
  assert.equal(
    rows.some((row) => row.name === 'neutral'),
    true
  );
});
...
test('portrait assets resolve exact names first and use only unambiguous normalized fallbacks', () => {
  const assets = codex.assetCatalog(
    {
      additionalAssets: [
        ['Wolf King.webp', 'asset-wolf', 'webp'],
        ['duplicate.png', 'asset-a', 'png'],
        ['DUPLICATE.PNG', 'asset-b', 'png']
      ],
      emotionImages: [
        ['angry', 'asset-angry'],
        ['smile', 'asset-smile']
  ...
  assert.equal(
    assets.some((row) => row.name === 'angry'),
    true,
    'emotion assets retain reserved catalog capacity'
  );
...
test('encounter portrait prefers standing defaults, then recent matching module assets when transport says NONE', () => {
  ...
  assert.equal(
    codex.assetForEntity(assets, amber, '<eomg="Amber_serious">old</eomg>\n<eomg="Amber_determined">now</eomg>')?.id,
    'assets/amber-standing.avif'
  );
  assert.equal(
    codex.assetForEntity(
      assets.filter((row) => !['Amber_standing', 'Amber_serious'].includes(row.name)),
      amber,
      '<eomg="Amber_serious">old</eomg>\n<eomg="Amber_determined">now</eomg>'
    )?.id,
    'assets/amber-determined.avif'
  );
  assert.equal(
    codex.assetForEntity(assets, { ...amber, portrait: 'Amber_serious' }, '<eomg="Amber_determined">now</eomg>')?.id,
    'assets/amber-serious.avif',
    'explicit transport remains authoritative'
  );
  assert.equal(
    codex.assetForEntity(assets, { id: 'unknown', name: '미상', aliases: [], portrait: 'NONE' }, '') ?? null,
    null
  );
});
...
test('large module catalogs keep later character portraits and ignore swimsuit standing false positives', () => {
  ...
  assert.ok(
    rows.some((row) => row.name === 'hakurei_reimu_default'),
    'catalog must keep characters after the old 400-name prefix'
  );
  assert.ok(rows.some((row) => row.name === 'kirisame_marisa_default'));
  assert.equal(
    codex.assetForEntity(rows, { id: 'reimu', name: '하쿠레이 레이무', aliases: [], portrait: 'NONE' })?.id,
    'asset-reimu-default'
  );
  assert.equal(
    codex.assetForEntity(
      rows,
      { id: 'meiling', name: '홍 메이링', aliases: ['hong meiling'], portrait: 'NONE' },
      '<img="hong_meiling_defeated">'
    )?.id,
    'asset-meiling-default'
  );
  assert.equal(
    codex.assetForEntity(rows, {
      id: 'kirisame_marisa',
      name: '키리사메 마리사',
      aliases: ['마리사'],
      portrait: 'NONE'
    })?.id,
    'asset-marisa-default'
  );
  ...
  assert.equal(
    codex.mentionedAssetNames('<img="hakurei_reimu_annoyed"> {{asset::hong_meiling_defeated}}').join(','),
    'hakurei_reimu_annoyed,hong_meiling_defeated'
  );
});
...
test('module portrait catalog includes active scopes only and supports namespace and bound persona assets', () => {
  const database = {
    ...
    moduleIntergration: 'integrated-ns',
    selectedPersona: 'persona-1',
    modules: [
  ...
  const rows = codex.activeModuleAssetCatalog(
    database,
    { modules: ['character-only'] },
    { modules: ['chat-only'], bindedPersona: 'persona-1' }
  );
  assert.deepEqual(JSON.parse(JSON.stringify(rows.map((row) => row.name).sort())), [
    'character.webp',
    'chat.webp',
    'global.webp',
    'integrated.webp',
  ...
  assert.equal(
    rows.some((row) => row.name === 'disabled.webp'),
    false
  );
});
...
test('re-exam preserves live skill and encounter state unless explicitly supplied', () => {
  const first = codex.extractResponse(
    '<skillExam><id>moon</id><name>월영참</name><glyph>🌙</glyph><rank>절정</rank><school>월광검법</school><type>passive</type><status>equipped</status><level>9</level><mastery>73</mastery><cost>월광 집중</cost><cooldown>호흡 안정 후</cooldown><target>단일</target><affinity>light</affinity><description>기존 설명</description><effects>참격 ;; 월광</effects><growth>극성</growth></skillExam><monsterExam><id>wolf</id><name>흑랑왕</name><glyph>🐺</glyph><aliases>검은 늑대</aliases><type>요수</type><threat>상</threat><relation>hostile</relation><status>active</status><portrait>wolf.webp</portrait><weaknesses>화염</weaknesses><resistances>암흑</resistances><moves>물어뜯기</moves><description>오래된 설명</description></monsterExam>',
    codex.snapshot()
  );
  const defeated = codex.extractResponse(
    '<monsterPatch><id>wolf</id><action>defeat</action></monsterPatch>',
    first.snapshot
  );
  const reexam = codex.extractResponse(
    '<skillExam><id>moon</id><name>월영참 개</name><description>새 설명</description></skillExam><monsterExam><id>wolf</id><name>흑랑왕 개</name><description>새 설명</description></monsterExam>',
    defeated.snapshot
  );
...
test('patch enums and conflicting operations are rejected before registry mutation', () => {
  const base = codex.extractResponse(
    '<skillExam><id>s1</id><name>기척 감지</name><type>passive</type></skillExam><monsterExam><id>m1</id><name>늑대</name><relation>hostile</relation><status>active</status></monsterExam>',
    codex.snapshot()
  ).snapshot;
  for (const raw of [
  ...
  assert.equal(
    codex.applyEvent(direct, {
      domain: 'monster',
      kind: 'patch',
      patch: { id: 'm1', action: null, op: 'merge', fields: { relation: 'banana' } }
    }),
    null
  );
...
test('monster patch state always remains inside its enum and active respects relation', () => {
  const first = codex.extractResponse(
    '<monsterExam><id>m1</id><name>기사</name><relation>neutral</relation><status>unknown</status></monsterExam>',
    codex.snapshot()
  );
  const activated = codex.extractResponse(
    '<monsterPatch><id>m1</id><op>merge</op><status>active</status></monsterPatch>',
    first.snapshot
  );
...
test('passive NONE patch is normalized with the existing skill type', () => {
  const first = codex.extractResponse(
    '<skillExam><id>sense</id><name>기척 감지</name><type>passive</type></skillExam>',
    codex.snapshot()
  );
  const patched = codex.extractResponse(
    '<skillPatch><id>sense</id><op>merge</op><cost>NONE</cost><cooldown>NONE</cooldown></skillPatch>',
    first.snapshot
  );
...
test('unsealing a sealed skill resolves both type and status', () => {
  const first = codex.extractResponse(
    '<skillExam><id>seal_art</id><name>봉인술</name><type>sealed</type><status>sealed</status><cost>NONE</cost><cooldown>NONE</cooldown></skillExam>',
    codex.snapshot()
  );
  const unsealed = codex.extractResponse(
    '<skillPatch><id>seal_art</id><action>unseal</action><cost>NONE</cost><cooldown>NONE</cooldown></skillPatch>',
    first.snapshot
  );
...
test('codex apply failures are error markers and never valid events', () => {
  const result = codex.extractResponse(
    '<monsterPatch><id>missing</id><action>defeat</action></monsterPatch>',
    codex.snapshot()
  );
  assert.equal(result.events.length, 0);
...
test('repeat skill scans cannot downgrade progress or erase detailed cost and cooldown', () => {
  const first = codex.extractResponse(
    '<skillExam><id>moon</id><name>월영참</name><type>active</type><level>40</level><mastery>88</mastery><cost>월광 30%</cost><cooldown>호흡이 완전히 안정된 뒤</cooldown></skillExam>',
    codex.snapshot()
  );
  const repeated = codex.extractResponse(
    '오랫동안 사용한 고수의 비전이다.\n<skillExam><id>moon</id><name>월영참</name><type>active</type><level>1</level><mastery>0</mastery><cost>NONE</cost><cooldown>none</cooldown></skillExam>',
    first.snapshot
  );
...
test('skill protocol uses real time cooldowns and world-native costs', () => {
  ...
  const result = codex.extractResponse(
    '<skillExam><id>flash</id><name>섬광</name><type>active</type><cost>기력 5%</cost><cooldown>30초</cooldown></skillExam>',
    codex.snapshot()
  );
  ...
  const legacy = codex.extractResponse(
    '<skillExam><id>legacy</id><name>구식 기술</name><type>active</type><cooldown>3턴</cooldown></skillExam>',
    codex.snapshot()
  );
  ...
  const explicitNone = codex.extractResponse(
    '<skillExam><id>free_cast</id><name>무영창</name><type>active</type><cost>NONE</cost><cooldown>none</cooldown></skillExam>',
    codex.snapshot()
  );
  ...
  const passive = codex.extractResponse(
    '<skillExam><id>sense</id><name>기척 감지</name><type>passive</type></skillExam>',
    codex.snapshot()
  );
...
test('qualitative skill evidence produces bounded progress while explicit novice values survive', () => {
  const veteran = codex.extractResponse(
    '오랫동안 숙련한 고수의 비전이다.\n<skillExam><id>old_art</id><name>고법</name><type>passive</type><level>1</level><mastery>0</mastery></skillExam>',
    codex.snapshot()
  );
  ...
  const baseline = codex.extractResponse(
    '<skillExam><id>quiet_art</id><name>고요한 호흡</name><type>passive</type></skillExam>',
    codex.snapshot(),
    { skillEvidenceText: '고요한 호흡을 사용할 수 있다.' }
  );
  ...
  const novice = codex.extractResponse(
    '방금 처음 배운 초보 검술이다.\n<skillExam><id>new_art</id><name>초보 검술</name><type>active</type><level>1</level><mastery>0</mastery></skillExam>',
    codex.snapshot()
  );
...
test('explicit progress patches replace inferred provenance without disturbing sibling fields', () => {
  const first = codex.extractResponse(
    '<skillExam><id>form</id><name>유운보</name><type>active</type></skillExam>',
    codex.snapshot(),
    { skillEvidenceText: '유운보를 구사할 수 있다.' }
  );
  assert.deepEqual([...first.snapshot.skills.entries.form._inferred].sort(), ['level', 'mastery']);
  const patched = codex.extractResponse(
    '<skillPatch><id>form</id><op>merge</op><mastery>82</mastery></skillPatch>',
    first.snapshot
  );
...
test('skill and encounter records derive safe emoji fallbacks and request free model-selected glyphs', () => {
  const result = codex.extractResponse(
    '<skillExam><id>flash</id><name>섬광</name><type>active</type></skillExam><monsterExam><id>wolf</id><name>늑대</name><relation>hostile</relation><status>active</status></monsterExam>',
    codex.snapshot()
  );
...
test('planning tag mentions cannot swallow the response or execute codex events', () => {
  ...
  for (const tag of ['Thoughts', 'Thought', 'think', 'thinking', 'DSThink', 'reasoning', 'analysis']) {
    ...
    assert.ok(result.content.endsWith(' 뒤 서술'));
    assert.equal(result.snapshot.monsters.entries.plateau_goblin_scout.name, '고원 고블린 척후');
    assert.equal(result.snapshot.skills.order.length, 0);
    const unfinished = `<${tag}>계획 ${monster}`;
    const ignored = codex.extractResponse(unfinished);
    assert.equal(ignored.content, unfinished);

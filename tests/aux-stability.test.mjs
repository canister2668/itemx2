...
async function bootWithOutput(data, options = {}) {
  let chat = {
    id: 'chat-guard',
    isStreaming: false,
    message: [...(options.prefixMessages || []), { role: 'char', data, metadata: { bgContinue: false } }],
...
test('periodic catch-up does not treat its own auxiliary marker as a fresh output', async () => {
  const output =
    '<itemExam><id>missing_blade</id><name>누락된 검</name><type>장검</type><emoji>⚔️</emoji><internalrarity>rare</internalrarity><possession>owned</possession><location>inventory</location></itemExam>';
...
test('periodic catch-up ignores host-added trailing whitespace after auxiliary commit', async () => {
  const output =
    '<itemExam><id>stable_blade</id><name>안정된 검</name><type>장검</type><emoji>⚔️</emoji><internalrarity>rare</internalrarity><possession>owned</possession><location>inventory</location></itemExam>';
  const result = await bootWithOutput('지훈은 안정된 검을 손에 넣었다.', {
    modelOutput: output,
    postWriteWhitespace: '\n\n\n\n'
  });
  assert.equal(result.modelCalls, 1);
...
test('auxiliary regeneration receives the triggering turn and recent visible narrative, not hidden thoughts or legacy ITEMX state', async () => {
  ...
  assert.match(
    prompt,
    /COMMITTED ASSISTANT OUTPUT \(visible narrative only\):[\s\S]*음검 모야를 뽑아 냉기와 공명시켰다/
  );
...
test('auxiliary treats the first confirmed already-owned player skill as a discovery event', async () => {
  const result = await bootWithOutput('동료는 챗붕이 [보법]을 실전에서 오랫동안 사용해 온 고수라고 확인했다.', {
    modelOutput:
      '<skillExam><id>footwork</id><name>보법</name><glyph>💨</glyph><type>passive</type><status>learned</status><description>실전에서 오랫동안 익힌 이동 기술.</description></skillExam>'
  });
  assert.equal(result.modelCalls, 1);
  assert.equal(result.chatWrites, 1);
  assert.match(
    result.prompts[0],
    /first discovery of an already-owned persistent player skill absent from CURRENT ACTIVE SKILLS/
  );
  assert.match(
    result.prompts[0],
...
test('detailed multi-item appraisal keeps safe partials and repairs them in one batch with one chat write', async () => {
  ...
  assert.deepEqual(
    state.abyssal_apocalypse.effects.map((one) => one.name),
    ['심연의 포식', '종말의 전조', '완전한 결속']
  );
...
test('failed partial repair commits once as partial_final and does not auto-retry', async () => {
  const initial =
    '<itemExam><id>scarred_blade</id><name>상흔의 검</name><type>장검</type><emoji>❔</emoji><internalrarity>epic</internalrarity><power>1500-3999</power><possession>owned</possession><location>inventory</location></itemExam>';
...
test('context-established item identity survives a pronoun-only committed output', async () => {
  const output =
    '<itemExam><id>black_iron_blade</id><name>흑철검</name><type>장검</type><emoji>🗡️</emoji><internalrarity>rare</internalrarity><possession>owned</possession><location>equipped</location></itemExam>';
...
test('unsupported auxiliary identity is recorded as rejected, never as no omissions', async () => {
  const output =
    '<itemExam><id>phantom_blade</id><name>없는검</name><type>장검</type><emoji>🗡️</emoji><internalrarity>rare</internalrarity><possession>owned</possession><location>inventory</location></itemExam>';

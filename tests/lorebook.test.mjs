...
test('explicit literal index preserves regex-lore identity without executing its regex', () => {
  ...
  const entry = {
    ...
    content: 'Private backstory stays private.',
    loreCache: {
      literalKeyIndex: {
        v: 1,
        keyDigest: (digest >>> 0).toString(16).padStart(8, '0'),
        keys: ['하쿠레이 레이무', '레이무']
      }
    }
  };
  ...
  assert.equal(
    lore.eligible({ ...entry, loreCache: { literalKeyIndex: { ...entry.loreCache.literalKeyIndex, keys: [null] } } }),
    false
  );
});
...
test('event-provided encounter fields outrank lorebook enrichment', () => {
  ...
  const scanned = lore.scan(
    base,
    [
      { id: 'lore', key: '하쿠레이 레이무', content: '[ITEMX-PUBLIC]\n종류: 인간\n초상화: lore_asset\n설명: 로어 설명' }
    ],
    lore.emptyLedger()
  );
...
test('ordinary lore prose is never copied into the visible encounter record', () => {
  ...
  const scanned = lore.scan(
    snapshot(),
    [{ id: 'secret', key: '하쿠레이 레이무', content: secret }],
    lore.emptyLedger()
  );
  const monster = lore.apply(snapshot(), scanned.ledger).monsters.entries.hakurei_reimu;
...
test('rescanning removes stale enrichment when its exact safe source disappears', () => {
  const first = lore.scan(
    snapshot(),
    [{ id: 'public', key: '하쿠레이 레이무', content: '[ITEMX-PUBLIC]\n종류: 인간 무녀' }],
    lore.emptyLedger()
  );
...
test('deleting the last encounter removes its enrichment without deleting other data', () => {
  const first = lore.scan(
    snapshot(),
    [{ id: 'public', key: '하쿠레이 레이무', content: '[ITEMX-PUBLIC]\n종류: 인간 무녀' }],
    lore.emptyLedger()
  );
  const next = lore.scan({ monsters: { order: [], entries: {} } }, [], first.ledger);
  assert.equal(next.result.removed, 1);
  assert.equal(Object.keys(next.ledger.rows).length, 0);
  assert.equal(Object.keys(first.ledger.rows).length, 1);
});

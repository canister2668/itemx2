...
test('exam provenance stays on the event but never leaks into registry or view', () => {
  const result = core.extractResponse(
    '<itemExam><id>plain_ore</id><name>무쇠 광석</name><type>재료</type><emoji>❔</emoji><possession>owned</possession><location>inventory</location></itemExam>',
    core.newRegistry()
  );
...
test('damage, stack consumption and disappearance are replayed deterministically', () => {
  ...
  const damaged = core.extractResponse(
    '<itemPatch><id>blade_a</id><op>merge</op><durability>31/100</durability></itemPatch>',
    first.registry
  );
  const destroyed = core.extractResponse(
    '[itemx: id=blade_a | action=destroy | quantity=all | reason=완전히 부서짐]',
    damaged.registry
  );
...
test('reappraising an existing id preserves stack, ownership, equipment and pin state', () => {
  const first = core.extractResponse(
    exam('blade_a', '시험검', '<count>3</count><slot>main_hand</slot><pin>true</pin>').replace(
...
test('a partial reappraisal preserves omitted authoritative item details', () => {
  const first = core.extractResponse(
    exam(
      'abyss_book',
      '심연의 묵시록',
      '<effects><effect><name>심연 개방</name><desc>봉인을 해제한다.</desc></effect></effects><augments><augment><name>+12</name><desc>극한 강화</desc></augment></augments>'
    ),
    core.newRegistry()
  );
  const partial = core.extractResponse(
    '<itemExam><id>abyss_book</id><name>심연의 묵시록 · 진본</name><type>마도서</type><possession>owned</possession><location>inventory</location></itemExam>',
    first.registry
  );
...
test('unknown operations and state-changing merge fields are rejected', () => {
  ...
  const merge = core.extractResponse(
    '<itemPatch><id>x</id><op>merge</op><location>equipped</location></itemPatch>',
    core.newRegistry()
  );
  assert.equal(merge.events.length, 0);
  assert.match(merge.errors[0], /merge_state/);
  const conflicting = core.extractResponse(
    '<itemPatch><id>x</id><action>consume</action><op>remove</op><quantity>1</quantity></itemPatch>',
    core.newRegistry()
  );
  ...
  core.applyEvent(direct, {
    kind: 'exam',
    item: { id: 'x', name: '시험품', possession: 'owned', location: 'inventory', count: 1 }
  });
  assert.equal(
    core.applyEvent(direct, {
      kind: 'patch',
      patch: { id: 'x', action: 'consume', op: 'remove', fields: {}, quantity: 1 }
    }),
    null
  );
...
test('a zero-count item cannot be consumed as a phantom unit', () => {
  ...
  const result = core.applyEvent(reg, {
    kind: 'patch',
    patch: { id: 'empty_stack', action: 'consume', op: null, fields: {}, quantity: 1 }
  });
...
test('apply failures are quarantined instead of being emitted as valid events', () => {
  const result = core.extractResponse(
    '<itemPatch><id>missing</id><action>equip</action><slot>main_hand</slot></itemPatch>',
    core.newRegistry()
  );
...
test('zero-count equipment and swap targets are rejected without mutation', () => {
  ...
  assert.equal(
    core.applyEvent(reg, {
      kind: 'patch',
      patch: { id: 'empty', action: 'equip', op: null, fields: {}, slot: 'off_hand' }
    }),
    null
  );
  ...
  assert.equal(
    core.applyEvent(reg, {
      kind: 'patch',
      patch: { id: null, action: 'swap', op: null, fields: {}, equip: 'empty', unequip: 'old', slot: 'main_hand' }
    }),
    null
  );
...
test('duplicate transform inputs are aggregated and failure is atomic', () => {
  const initial = core.extractResponse(
    `${exam('ore', '광석', '<count>3</count>')}\n${exam('ingot', '주괴', '<count>0</count>')}`,
    core.newRegistry()
  );
  const result = core.extractResponse(
    '<itemPatch><action>transform</action><inputs>ore:2,ore:2</inputs><outputs>ingot:1</outputs></itemPatch>',
    initial.registry
  );
  assert.equal(result.events.length, 0);
...
test('rarity particle budgets keep rare and below restrained', () => {
  assert.deepEqual(
    { ...renderer.particleBudget },
    { normal: 4, magic: 6, rare: 8, unique: 16, epic: 16, legendary: 16, mythical: 16, empyrean: 16 }
  );
});
...
test('moxie observed acquisition precedes equip and planning never executes', () => {
  const reg = core.extractResponse(
    exam('moxie_sword', '막야').replace('<possession>owned</possession>', '<possession>observed</possession>'),
    core.newRegistry()
  ).registry;

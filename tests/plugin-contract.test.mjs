...
test('built ITEMX CODEX plugin is API v3 and owns both UI and pipeline hooks', async () => {
  ...
  assert.match(source, /^\/\/@version 1\.9\.0-beta\.15$/m);
  assert.match(source, /^\/\/@display-name ITEMX CODEX · v1\.9\.0-beta\.15$/m);
  ...
  assert.match(source, /const ITEMX_VERSION_LABEL = '1\.9 · BETA 15'/);
  ...
  assert.ok(source.length < 425000, 'plugin bundle must stay compact with transport stripping, themed CODEX FX and font controls');

...
test('built ITEMX CODEX plugin is API v3 and owns both UI and pipeline hooks', async () => {
  ...
  assert.match(source, /^\/\/@version 1\.9\.0-beta\.16$/m);
  assert.match(source, /^\/\/@display-name ITEMX CODEX · v1\.9\.0-beta\.16$/m);
  ...
  assert.match(source, /const ITEMX_VERSION_LABEL = '1\.9 · BETA 16'/);
  ...
  assert.match(source, /itemx-monster-portrait\{filter:none\}/);
  assert.match(source, /\.itemx2-effects-off \.itemx2-codex-fx\{display:none!important/);
  assert.match(source, /@media\(prefers-reduced-motion:reduce\)\{\.itemx2-codex-fx,\.itemx2-codex-fx \*\{animation:none!important\}\}/);
  ...
  assert.equal(materialFx.includes('itemx2-codex-spin'), false);

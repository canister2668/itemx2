...
test('built ITEMX CODEX plugin is API v3 and owns both UI and pipeline hooks', async () => {
  ...
  assert.match(source, /^\/\/@version 2\.0\.2$/m);
  assert.match(source, /^\/\/@display-name ITEMX CODEX · v2\.0\.2$/m);
  ...
  assert.match(source, /const ITEMX_VERSION_LABEL = "2\.0\.2"/);
  ...
  assert.ok(source.length < 480000, 'plugin bundle must stay compact with lazy detail hydration and fixed-layer inline appraisal cards');
